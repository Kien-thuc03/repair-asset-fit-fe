...
// Ngày tạo: 2025-10-02
import { SoftwareProposal, SoftwareProposalItem, SoftwareProposalStatus } from '@/types/software';
...
export const mockSoftwareProposals: SoftwareProposal[] = [
  {
    ...
    reason: 'Cần cài đặt Adobe Creative Suite để phục vụ môn Thiết kế đồ họa. Phần mềm hiện tại đã lỗi thời và không đáp ứng yêu cầu giảng dạy.',
  ...
  },
  {
    ...
    reason: 'Yêu cầu cài đặt Microsoft Office 365 mới nhất cho 30 máy tính phục vụ thực hành tin học văn phòng.',
  ...
  },
  {
    ...
    reason: 'Cần cài đặt Visual Studio 2022, IntelliJ IDEA và các công cụ lập trình để phục vụ môn Lập trình nâng cao.',
    status: SoftwareProposalStatus.ĐÃ_DUYỆT,
  ...
  },
  {
    ...
    reason: 'Đề xuất cài đặt phần mềm chỉnh sửa video chuyên nghiệp Adobe Premiere Pro cho tất cả máy tính.',
  ...
  },
  {
    ...
    reason: 'Cài đặt trình duyệt web và các phần mềm cơ bản cho phòng thực hành mới.',
...
// Helper functions
export const getSoftwareProposalById = (id: string): SoftwareProposal | undefined => {
  return mockSoftwareProposals.find(proposal => proposal.id === id);
};

export const getSoftwareProposalItems = (proposalId: string): SoftwareProposalItem[] => {
  return mockSoftwareProposalItems.filter(item => item.proposalId === proposalId);
};

export const getSoftwareProposalsByStatus = (status: SoftwareProposal['status']): SoftwareProposal[] => {
  return mockSoftwareProposals.filter(proposal => proposal.status === status);
};

export const getSoftwareProposalsByProposer = (proposerId: string): SoftwareProposal[] => {
  return mockSoftwareProposals.filter(proposal => proposal.proposerId === proposerId);
};