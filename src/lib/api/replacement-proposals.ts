...
/**
 * Interface cho component từ repair request (available components)
 */
...
/**
 * Interface cho query parameters của available components
 */
export interface GetAvailableComponentsQueryParams {
  ...
  // Sorting
  sortBy?: "createdAt" | "componentName" | "assetName" | "requestCode";
  sortOrder?: "ASC" | "DESC";
}

/**
 * Interface cho response của available components
 */
export interface GetAvailableComponentsResponse {
  data: ComponentFromRepair[];
  total: number;
  page: number;
  limit: number;
  totalPages: number;
...
/**
 * Lấy danh sách linh kiện từ yêu cầu sửa chữa mà kỹ thuật viên đảm nhận
 * API này chỉ trả về các linh kiện từ repair requests được phân công cho kỹ thuật viên hiện tại
 */
export const getAvailableComponents = async (
  params?: GetAvailableComponentsQueryParams
): Promise<GetAvailableComponentsResponse> => {
  try {
    console.log("🌐 API Call: GET /api/v1/replacement-proposals/current-user/components-from-repair", params);
    const response = await api.get<GetAvailableComponentsResponse>(
      "/api/v1/replacement-proposals/current-user/components-from-repair",
      { params }
    );
    console.log("✅ API Response:", response.data);
    return response.data;
