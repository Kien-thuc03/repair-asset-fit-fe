...
import { Clock, CheckCircle, AlertTriangle, Eye, Search, X } from 'lucide-react'
...
const mockRequests: RepairRequest[] = [
  {
    ...
    description: 'Máy tính không khởi động được, có tiếng beep liên tục khi bấm nút nguồn. Đèn LED mainboard vẫn sáng bình thường.',
    status: 'ĐANG_XỬ_LÝ',
  ...
  },
  {
    ...
    description: 'Màn hình không hiển thị hình ảnh, đèn nguồn vẫn bật nhưng màn hình đen. Máy tính khởi động bình thường.',
    status: 'HOÀN_THÀNH',
    resolutionNotes: 'Đã thay thế cáp VGA và cập nhật driver card màn hình. Màn hình hoạt động bình thường.',
  ...
  },
  {
    ...
    description: 'Máy tính khởi động và chạy rất chậm, mở ứng dụng lâu, thường xuyên bị đơ.',
  ...
  },
  {
    ...
    description: 'Một số phím trên bàn phím không hoạt động, đặc biệt là phím Space và Enter.',
    status: 'ĐANG_XỬ_LÝ',
...
export default function TheoDaoTienDoPage() {
  ...
  const [filteredRequests, setFilteredRequests] = useState<RepairRequest[]>(mockRequests)
      ...
      <div className="bg-white shadow rounded-lg p-6">
        <div className="flex items-center space-x-3">
          ...
          <div>
            ...
            <p className="text-gray-600">Theo dõi trạng thái xử lý các báo cáo đã gửi</p>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white shadow rounded-lg overflow-hidden">
              ...
              {filteredRequests.map((request) => {
                ...
                return (
                  <tr key={request.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium border ${statusConfig[request.status].color}`}>
        ...
        {filteredRequests.length === 0 && (
          <div className="text-center py-12">
            ...
            <h3 className="mt-2 text-sm font-medium text-gray-900">Không có yêu cầu</h3>
              ...
              <div className="space-y-4">
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <p className="mt-1 text-sm text-gray-900">{selectedRequest.assetName}</p>
                    <p className="text-sm text-gray-500">{selectedRequest.assetCode}</p>
                    {selectedRequest.componentName && (
                      <p className="text-xs text-gray-400">Linh kiện: {selectedRequest.componentName}</p>
                    )}
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-gray-700">Phòng</label>
                    <p className="mt-1 text-sm text-gray-900">{selectedRequest.roomName}</p>
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-gray-700">Loại lỗi</label>
                    <p className="mt-1 text-sm text-gray-900">{selectedRequest.errorTypeName || 'Chưa phân loại'}</p>
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-gray-700">Trạng thái</label>
                    <div className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium border mt-1 ${statusConfig[selectedRequest.status].color}`}>
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  {selectedRequest.acceptedAt && (
                    <div>
                      <label className="block text-sm font-medium text-gray-700">Ngày tiếp nhận</label>
                      <p className="mt-1 text-sm text-gray-900">{formatDate(selectedRequest.acceptedAt)}</p>
                    </div>
                  )}
                  {selectedRequest.completedAt && (
                    <div>
                      <label className="block text-sm font-medium text-gray-700">Ngày hoàn thành</label>
                      <p className="mt-1 text-sm text-gray-900">{formatDate(selectedRequest.completedAt)}</p>
                    </div>
                  )}
                </div>
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <p className="mt-1 text-sm text-gray-900">{selectedRequest.reporterName}</p>
                  </div>
                  {selectedRequest.assignedTechnicianName && (
                    <div>
                      <label className="block text-sm font-medium text-gray-700">Người xử lý</label>
                      <p className="mt-1 text-sm text-gray-900">{selectedRequest.assignedTechnicianName}</p>
                    </div>
                  )}
                </div>
                ...
                {selectedRequest.resolutionNotes && (
                  <div>
                    ...
                    <p className="mt-1 text-sm text-gray-900">{selectedRequest.resolutionNotes}</p>
                  </div>
                )}
              </div>
            </div>
