...
export default function QuanLyThayTheLinhKienPage() {
  ...
  const {
    ...
    error,
    refetch,
  } = useReplacementProposalsByProposer(userDetails?.id || null);
  ...
  const handleExportExcel = async () => {
    ...
    try {
      ...
      const worksheet = workbook.addWorksheet("Danh sách đề xuất");
      ...
      cell4.value = "DANH SÁCH ĐỀ XUẤT THAY THẾ LINH KIỆN";
      cell4.font = {
        name: "Arial",
        size: 12,
        bold: true,
    ...
    <div className="space-y-6">
      ...
      <div>
        ...
        <p className="mt-2 text-gray-600">
          Quản lý và theo dõi các đề xuất thay thế linh kiện đã được tạo. Đối
          với các đề xuất Đã hoàn tất mua sắm, bạn có thể xem chi tiết đề xuất
          và thực hiện thay thế linh kiện.
        </p>
      </div>
      ...
      <div className="bg-white p-4 rounded-lg shadow space-y-4">
        <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
          ...
          <Select
            ...
            <Option value={ReplacementProposalStatus.ĐÃ_DUYỆT}>Đã duyệt</Option>
          ...
          <>
            <table className="min-w-full divide-y divide-gray-200">
                    ...
                    <div className="flex items-center space-x-2">
                      <input
                        ...
                        checked={
                          ...
                          paginatedData.every((row: ReplacementProposal) =>
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {paginatedData.map(
                  (record: ReplacementProposal, index: number) => {
                    const config = getReplacementProposalStatusConfig(
                      record.status
                    );
                    return (
                      <tr key={record.id} className="hover:bg-gray-50">
                        ...
                        <td className="px-4 py-3 whitespace-nowrap text-sm font-medium text-blue-600 hover:text-blue-800 hover:underline">
                          <Link
                            href={`/ky-thuat-vien/quan-ly-thay-the-linh-kien/chi-tiet/${record.id}`}>
                            {record.proposalCode}
                          </Link>
                        </td>
                        ...
                        <td className="px-4 py-3 whitespace-nowrap text-sm text-gray-700 text-center">
                          <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-blue-100 text-blue-800">
                            {record.items?.length || record.itemsCount || 0}{" "}
                            linh kiện
                          </span>
                        </td>
