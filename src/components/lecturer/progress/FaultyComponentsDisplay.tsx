...
export default function FaultyComponentsDisplay({ request }: FaultyComponentsDisplayProps) {
  if (!request.faultyComponents || request.faultyComponents.length === 0) {
    return (
      <Card title="Linh kiện bị lỗi" className="mt-4">
        <p className="text-gray-500">Chưa có thông tin chi tiết về linh kiện bị lỗi.</p>
      </Card>
    );
  }
    ...
    <Card title="Chi tiết linh kiện bị lỗi" className="mt-4">
      <div className="space-y-4">
        {request.faultyComponents.map((component, index) => {
          const IconComponent = componentIcons[component.componentType as keyof typeof componentIcons] || HardDrive;
          const color = componentTypeColors[component.componentType as keyof typeof componentTypeColors] || 'default';
          
          return (
            <div key={component.componentId} className="border border-gray-200 rounded-lg p-4">
      ...
      <div className="mt-4 p-3 bg-blue-50 border border-blue-200 rounded">
        <p className="text-sm text-blue-800">
          <strong>Tổng số linh kiện bị lỗi:</strong> {request.faultyComponents.length} linh kiện
        </p>
      </div>
    </Card>
  );
